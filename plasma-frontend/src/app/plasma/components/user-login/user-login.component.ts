...
private password: string;
  ...
  login(event: any) {
    ...
    this.loginService.login(this.userAuth).subscribe((res: any) => {
      this.snackBar.open('User logged in successfully', '', { duration: 5000, verticalPosition: 'bottom' });
      console.log('Res: ', res);
      ...
      this.isLoggedIn = true;
       ...
       err => {
        ...
        if (this.statusCode === 401) {
         ...
         this.snackBar.open('User does not exist !!!', '', { duration: 5000, verticalPosition: 'bottom' });
        // window.alert('user');
        }
    });
  }
